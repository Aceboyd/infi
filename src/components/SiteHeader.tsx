...
export default function SiteHeader() {
  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-[100] flex w-full items-center justify-between border-b border-white/25 bg-[#101513]/45 px-6 py-5 shadow-lg backdrop-blur-md lg:px-10">
      <Logo light />
      <div className="hidden items-center gap-7 text-xs text-white md:flex">
      ...
      <div className="flex items-center gap-3"><Link href="/sign-in" className="hidden px-4 py-2 text-xs text-white hover:text-[#f7d8c3] sm:block">Sign in</Link><Link href="/sign-up" className="hidden border border-[#f7d8c3] px-4 py-2.5 text-xs text-[#fffaf4] transition hover:bg-[#c9754d] hover:text-white sm:block">Begin your plan <ArrowRight className="ml-2 inline" size={14} /></Link><button onClick={() => setMobileOpen(!mobileOpen)} className="text-white md:hidden" aria-label="Toggle navigation">{mobileOpen ? <X size={21} /> : <Menu size={21} />}</button></div>
